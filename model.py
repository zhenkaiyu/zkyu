...
class Multicnns:
    ...
    def build(self):
        with tf.name_scope('placeholder'):
            ...
            self.entity_ph = tf.placeholder(dtype=tf.int32, shape=(None, self.sequence_length), name='entity')
        ...
        with tf.name_scope('embedding'):
            ...
            self.embedded_entitys = tf.nn.embedding_lookup(self.en,self.entity_ph)
            embedding.append(self.feature_embedding)
            embedding.append(self.embedded_entitys)
            embed = tf.concat(embedding, axis=-1)

            # (bs,sl,d,1)
            feature_embedding_expand = tf.expand_dims(embed, axis=-1)
            # 此处的dropout表示保留的比例
    ...
    def fit(self,x_train,y_train,x_dev,y_dev,entity_train,entity_dev):
        max_acc = 0.
        current_patience = 0
        ...
        batches = batch_generator(x_train, y_train,entity_train,self.batch_size)
        ...
        with tf.Session() as sess:
            ...
            for step in range(self.num_epochs):
                ...
                for _ in range(batch_num_in_epoch):
                    ...
                    feed_dict[self.entity_ph] = entity_batch
                    ...
                    if global_step_%100==0:
                        ...
                        acc_dev,y_pre,y=self.predict(x_dev,y_dev,entity_dev,self.batch_size,sess,self.logits)
                        ...
                        if max_acc<acc_dev:
                            ...
                            f = open("data/y_pre.txt", "w", encoding="utf-8")
                            f1 = open("data/y.txt", "w", encoding="utf-8")
                ...
                acc_train,train_y_pre,train_y=self.predict(x_train,y_train,entity_train,self.batch_size,sess,self.logits)
    ...
    def predict(self,x_data,y,entity,batch_size,sess,scores):
        all_predict=[]
        data_count=len(x_data)

        batches=batch_generator_test(x_data,y,entity,self.batch_size)
...
if __name__=='__main__':
    ...
    if language=='chinese':
        ...
        embedding=np.array(np.load(os.path.join(ch_rpath,'embedding.npy')),dtype=np.float32)
        # ltp maxlen=32
        # config['sequence_length']=32
        # jieba maxlen=30
        config['sequence_length']=30
        # config['model_path']='/home/fhzhu/youqianzhu/newstitle/model/labelbased/cnn_model/bestmodel'
        config['model_path']='data/model/labelbased/new_model/cnn_max_avg/bestmodel'
        x_train = np.load(os.path.join(ch_rpath, 'x_train.npy'))
        x_dev=np.load(os.path.join(ch_rpath,'x_dev.npy'))
    ...
    entity_train = np.load(os.path.join(tag_path, 'entity_train.npy'))
    entity_dev = np.load(os.path.join(tag_path, 'entity_dev.npy'))
    ...
    multicnn.fit(x_train,y_train,x_dev,y_dev,entity_train,entity_dev)